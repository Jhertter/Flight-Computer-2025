...
bool SFE_UBLOX_GPS::checkUbloxI2C(ubxPacket *incomingUBX, uint8_t requestedClass, uint8_t requestedID)
{
    if (to_ms_since_boot(get_absolute_time()) - lastCheck >= i2cPollingWait)
    {
        ...
        uint8_t reg = 0xFD;
        if (i2c_write_blocking(_i2cPort, _gpsI2Caddress, &reg, 1, false) != 0) // Send a restart command. Do not release bus.
        ...
        {
            uint8_t msb = 0;
            i2c_read_blocking(_i2cPort, _gpsI2Caddress, &msb, 1, false); // Send a restart command. Do not release bus.
            uint8_t lsb = 0;
        ...
        {
            reg = 0xFF;
            if(i2c_write_blocking(_i2cPort, _gpsI2Caddress, &reg, 1, false)); // Send a restart command. Do not release bus.
                return (false);                        // Sensor did not ACK
        ...
        TRY_AGAIN:
            ...
            if (i2c_get_read_available(_i2cPort))
            {
                ...
                {
                    uint8_t incoming = 0;
                    i2c_read_blocking(_i2cPort, _gpsI2Caddress, &incoming, 1, false); // Send a restart command. Do not release bus.
...
sfe_ublox_status_e SFE_UBLOX_GPS::sendI2cCommand(ubxPacket *outgoingUBX, uint16_t maxWait)
{
    ...
    if (i2c_write_blocking(_i2cPort, _gpsI2Caddress, reg, 6, false)) // Send header bytes
        return (SFE_UBLOX_STATUS_I2C_COMM_FAILURE); // Sensor did not ACK
    ...
    while (bytesToSend > 1)
    {
        ...
        if (i2c_write_blocking(_i2cPort, _gpsI2Caddress, reg, len, false) != 0)      // Don't release bus
            return (SFE_UBLOX_STATUS_I2C_COMM_FAILURE); // Sensor did not ACK
    ...
    if(i2c_write_blocking(_i2cPort, _gpsI2Caddress, reg, 2+size, false)) // Send checksum bytes   
        return (SFE_UBLOX_STATUS_I2C_COMM_FAILURE); // Sensor did not ACK
